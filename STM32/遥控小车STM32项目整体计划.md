...
| 模块 | 最小闭环 | 验收现象 |
|---|---|---|
| 电机驱动 | `TIM3 PWM` + `PB12~PB15` 方向 GPIO | A/B 两路电机能前进、后退、刹车，占空比变化有效 |
| 蓝牙遥控 | `USART1` 接收 `[F/B/L/R/S]` 或扩展协议 | 手机端指令能稳定映射为目标速度和转向量 |
| T 法测速 | `TIM2/TIM4` 输入捕获 + B 相方向 GPIO | 转动单轮时 `period_us`、方向和 `rpm` 符号合理 |
| 速度闭环 | 左右轮各一个速度 PID | 目标速度变化后，左右轮能跟踪且不过度振荡 |
| 姿态/航向 | `MPU6050` + 互补滤波 / `gyro_z` 积分 | 短时间转向修正有效，长期 yaw 漂移不当成绝对方向 |
| OLED/调试 | 100ms 左右刷新显示状态 | 能看到目标速度、实际速度、PWM、关键调参值 |

核心调用关系保持为：

```text
TIM1 1ms 中断只置标志
main loop -> AppTask_Run()
5ms  -> 读取遥控/姿态快变量
10ms -> T 法测速 + 速度 PID + 电机输出
100ms -> OLED 刷新 + 蓝牙调试输出
```

## 14. 验收自测题

1. 为什么角度/航向环不直接输出 PWM，而是输出左右轮速度差 `turn_comp`？
2. TIM3 在 APB1 分频为 2 时，为什么定时器输入时钟仍按 `72MHz` 计算？
3. T 法测速为什么适合低速？高速时它相比 M 法会遇到什么限制？
4. `MPU6050` 没有磁力计时，为什么不能把 `yaw` 当长期绝对航向？
5. 如果左右轮同样目标速度但车持续跑偏，应该按“电机方向、测速符号、PID 输出符号、机械差异”的什么顺序排查？
6. OLED 刷新和蓝牙调试输出为什么不能放进高频中断？
