...
我们可以看到下面的例子，如图所示：
![alt text](./image/ch04-image-1.png)
...
我们可以看到下面的例子：
![alt text](./image/ch04-image-2.png)
...
![alt text](./image/ch04-image-3.png)
...
![alt text](./image/ch04-image-4.png)
```python
import heapq  
  
...
![alt text](./image/ch04-image-5.png)
```python
...
```
![alt text](./image/ch04-image-6.png)
...
```
得到结果如下图所示：
![alt text](./image/ch04-image-7.png)
...
![alt text](./image/ch04-image-23.png)
...
![alt text](./image/ch04-image-24.png)

首先，找到从S到T的一条最大流量，由于总流量会存在木桶效应，路径中最小的流量决定了这条路上通过的总流量。我们把红线上的流量都-3以后添加反向边。

![alt text](./image/ch04-image-25.png)

继续找最大的一条流量通路，同样进行增加反向边的操作。这时我们发现有两条反向边可以合并。

![alt text](./image/ch04-image-26.png)

重复上述操作，此时终于产生了两个连通分量。

![alt text](./image/ch04-image-27.png)

最后一次，完成。

![alt text](./image/ch04-image-28.png)

最终得到的结果如下。

![alt text](./image/ch04-image-29.png)
...
```
![alt text](./image/ch04-image-8.png)
...
![alt text](./image/ch04-image-9.png)
...
![alt text](./image/ch04-image-10.png)
- 容量限制VRP

![alt text](./image/ch04-image-11.png)

- 时间窗VRP

![alt text](./image/ch04-image-12.png)
...
![alt text](./image/ch04-image-13.png)

![alt text](./image/ch04-image-14.png)
...
![alt text](./image/ch04-image-15.png)
