...
本章讨论数学建模中“拿到数据之后该怎么做”的问题：先理解数据形态，再完成清洗、规约、统计分析、随机模拟、可视化表达和插值填补。建模竞赛中的很多题目并不是直接给出一个标准模型，而是先给一批杂乱的数据，让我们从数据中提取结构、验证假设、形成结论。

本章主线可以概括为：

1. 明确数据是什么：数据不只是一张 Excel 表，也可以是文本、图像、音频、信号和多模态信息。
2. 处理原始数据：用 pandas 完成缺失值、重复值、字段拆分、格式清洗和规约。
3. 选择分析模型：根据变量类型和研究目标，在回归、分类、假设检验、方差分析、随机模拟等方法之间做选择。
4. 表达模型结果：用合适的图表展示比较、关系、分布和时间变化。
5. 填补缺失或提高分辨率：用线性插值、样条插值、拉格朗日插值等方法构造连续近似。
...
> 注意：视频没有单独列为一类，是因为视频本质上由连续图像帧构成；如果包含声音，还会叠加音频信息。因此，视频可以看作图像、音频与时间序列共同组成的数据形态。
### 6.1.2  数据与大数据
...
![alt text|686](img/docs-CH6-attachment-image.png)
...
> 注意：本节主要用于澄清概念。后续介绍数据处理方法时，仍会以 Excel 表格类数据为主要例子；图像、文本等非常规数据会在第 10 章单独介绍。`csv`、`tsv` 等文件可以用记事本、Excel、WPS 或代码编辑器打开。
...
![alt text](img/docs-CH6-attachment-image-1.png)
...
（3）返回 `DataFrame df` 的前 5 行数据：

```python
df.head(5)
```

（4）从 `DataFrame df` 中选择 `animal` 和 `age` 两列：

```python
df[['animal', 'age']]
```

（5）在 `[3, 4, 8]` 行中，选择 `animal` 和 `age` 两列：
...
（8）选择 `animal` 是 `cat` 且 `age` 小于 3 的行：
...
（12）计算每种 `animal` 的平均 `age`：

```python
df.groupby(['animal'])['age'].mean()
```

### 6.2.3  使用 pandas 处理数据的进阶
6.2.2 节主要介绍 pandas `DataFrame` 的基础语法。实际建模中更常遇到的是字段拆分、缺失值填补、文本清洗、列表列展开、增加样本和去重等任务。下面用一个小例子串联这些操作。
```python
...
# （2）FlightNumber 列中有某些缺失值，缺失值常用 nan 表示，请在该列中添加 10055 与 10075 填充该缺失值。
df['FlightNumber'] = df['FlightNumber'].interpolate().astype(int)

# （3）由于列 From_To 代表从地点 A 到地点 B，因此可以将这列拆分成两列，并赋予为列 From 与 To。
temp = df['From_To'].str.split("_", expand=True)
temp.columns = ['From', 'To']

# （4）将列 From 和 To 转化成只有首字母大写的形式。
temp['From'] = temp['From'].str.capitalize()
temp['To'] = temp['To'].str.capitalize()

# （5）将列 From_To 从 df 中去除，并把列 From 和 To 添加到 df 中。
...
# （7）在 RecentDelays 列中，值已作为列表输入到 DataFrame 中。将 Series 列表展开为名为 delays 的 DataFrame，重命名列 delay_1、delay_2 等，并用 delays 替换不需要的 RecentDelays 列。
...
# （8）将 delay_i 列中的 NaN 都填为自身的平均值。
for i in range(1, 4):
    df[f'delay_{i}'] = df[f'delay_{i}'].fillna(np.mean(df[f'delay_{i}']))

# （9）在 df 中增加一行，值与 FlightNumber=10085 的行保持一致。
...
### 6.2.4  数据的规约
数据为何需要规约？因为实际应用中数据的分布可能是有偏的，量纲影响和数值差异可能会比较大。规约是为了形成对数据的更高效表示，学习到更好地模型。它会保留数据的原始特征，但对极端值、异常值等会比较敏感。这里我们就介绍两个比较典型的规约方式：min-max规约和Z-score规约。
...
$$
本质上，一列数据减去其均值再除以标准差，如果这一列数据近似服从正态分布，这个过程就是化为标准正态分布的过程。Z-score 规约和 min-max 规约往往不是二者取其一，有时候两个可以组合起来用。除此以外，还有很多的规约方法，但是这两种最为常用。若使用 Python 实现，可以直接调用 `sklearn.preprocessing`：
...
df_minmax = minmax_scaler.fit_transform(df[cols])
df_zscore = zscore_scaler.fit_transform(df[cols])
```

使用时要注意：如果某一列包含极端异常值，min-max 规约会被异常值强烈拉伸；如果后续模型对正态性较敏感，Z-score 规约通常更容易解释。竞赛中不要机械套用规约方法，要先看变量含义、分布形态和异常值情况。
...
| 任务目标 | 常见方法 | 关键判断 |
| --- | --- | --- |
| 解释连续因变量与自变量的关系 | 回归分析 | 因变量是否连续，系数是否可解释，残差是否合理 |
| 判断样本属于哪一类 | 分类分析 | 因变量是否为类别变量，关注预测概率还是分类结果 |
| 判断总体性质是否满足某个命题 | 假设检验 | 原假设、备择假设、显著性水平和样本条件是否明确 |
| 比较多组均值是否相同 | 方差分析 / Kruskal-Wallis 检验 | 组数、正态性、方差齐性和样本量 |
| 模拟随机系统随时间变化 | 随机过程 / 随机模拟 | 是否存在随机到达、状态转移、排队或累计过程 |
...
```
![](img/docs-CH6-attachment-image-2.png)

从模型 I 可见，ACT 变量的 `pvalue` 为 0.01，小于 0.05，说明大学入学考试成绩 ACT 能显著影响大学 GPA；但从模型 II 可见，高考成绩 hsGPA 的 `pvalue` 为 0.000005，远小于 0.05，说明高考成绩 hsGPA 显著影响大学成绩 GPA，而大学入学考试成绩 ACT 的 `pvalue` 为 0.38，大于 0.05，不能说明大学入学考试成绩 ACT 显著影响大学成绩 GPA。为什么会出现这样的差异？原因可能是：在没有加入高考成绩 hsGPA 时，大学入学考试成绩 ACT 确实能显著影响大学成绩 GPA；但是加入高考成绩 hsGPA 后，可能高考成绩 hsGPA 高的同学大学入学考试也高分，两者相关性较高，于是 ACT 的独立解释作用就不明显了。

从模型 II 的系数 `params` 数值可见，当高考成绩 hsGPA 显著影响大学成绩 GPA 时，`params` 为 0.45，代表高考成绩每增加 1 个单位，大学 GPA 平均增加 0.45 个单位。

（2）分类分析：ST 是我国股市特有的一项保护投资者利益的制度。当上市公司因财务状况不佳导致投资者难以预测其前景时，交易所会标记该公司股票为 ST，并进行一系列限制措施。我们想研究被 ST 的公司背后的因素，并尝试利用公司的财务指标提前预测某上市公司在未来是否会被 ST。在这项任务中，因变量就是公司是否会被 ST。该例中自变量是一些财务指标，如 ARA、ASSET 等，它们具体的意义在此不做赘述。
```python
# 加载基础包
import pandas as pd
import numpy as np
...
```
![alt text](img/docs-CH6-attachment-image-3.png)

其中 `pvalue` 为第四列标记为 `P(>|z|)` 的数值。与回归分析类似，一般认为，p 值小于 0.05 的自变量是显著的，也就是说，有统计学证据表明该变量会影响因变量为 1 的概率。
...
为了便于读者理解，我们用一个例子开始对这个话题的讨论：必胜中学初三年级在三天前开展了一次数学考试，校长给级长下达了命令：全级均分必须不低于 110，否则就要扣级长工资。今天考试成绩出来了，但是由于年级人数太多，完成全级考试情况的统计还需要两天的时间。急性子的级长很想早点知道这次考试的情况，万幸的是一班已经完成了成绩统计，级长便想以一班的成绩，推测出全级的考试情况。已知：一班的成绩均值为 108.2，样本标准差 $s=4$，人数 $n=25$，且根据以往的经验，年级的考试成绩呈现正态分布。那么，级长可以认为年级均分不低于 110 吗？
...
（1）正态性检验：由于参数检验通常比非参数检验更灵敏，因此一旦数据可以认为近似正态，我们应优先考虑参数检验。此时，对数据进行正态性检验就非常有必要。这里提供三种帮助判断正态性的方法：可视化判断（正态概率图）、Shapiro-Wilk 检验、D'Agostino's K-squared 检验。
```python
# 生成1000个服从正态分布的数据
...
```
![alt text](img/docs-CH6-attachment-image-4.png)

可见，正态性数据在正态分布概率图中十分接近直线y=x，而卡方分布数据则几乎完全偏离了该直线。可见，概率图可视化确实可以起到初步判断数据正态性的作用。实际应用中，由于数据的复杂性，仅使用一种方法判断正态性有可能产生一定的误差，因此我们通常同时使用多种方法进行判断。如果不同方法得出的结论不同，此时就需要仔细观察数据的特征，寻找结果不一致的原因。如：若Shapiro-Wilk test显著（非正态），D'Agostino's K-squared test不显著（正态），则有可能是因为样本量较大，或者样本中存在重复值现象，如果事实确实如此，那么我们就应该采纳D'Agostino's K-squared test的结论而非Shapiro-Wilk test的结论。（在这里，我们假设数据服从正态分布，p<0.05则拒绝假设，换个说法：p<0.05数据不服从正态分布，如果p>=0.05说明没有证据说明数据不服从正态分布）
```python
...
```
![alt text](img/docs-CH6-attachment-image-5.png)
```text
...
def check_mean(data,checkvalue,significance=0.05,alternative='two-sided'):
    ...
    if pVal['t-test'] < significance:
      ...
      print(('在显著性水平{1:}下，无法拒绝总体均值等于目标值{0:4.2f}的假设.(p={2:5.3f})'.format(checkvalue,significance,pVal['t-test'])))
    ...
    if pVal['wilcoxon'] < significance:
      ...
      print(('在显著性水平{1:}下，无法拒绝总体均值等于目标值{0:4.2f}的假设.(p={2:5.3f})'.format(checkvalue,significance,pVal['wilcoxon'])))
...
```text
t-test------------------------
在显著性水平0.05下，无法拒绝总体均值等于目标值137.00的假设.(p=1.000)
wilcoxon------------------------
在显著性水平0.05下，无法拒绝总体均值等于目标值137.00的假设.(p=0.812)
...
为了让读者对这里的“独立”有较好的了解，我们看看下面的例子。陈老师在年级有一个竞争对手：王老师。王老师在他们班级在同一时间也举行了一次英语考试，且两个班用的是同一份卷子，因此两个班的英语成绩就具有比较的意义了。和陈老师一样，王老师也来不及批改和统计他们班级的英语成绩，不过他手头上也有12份已经改好的试卷，成绩分别为：
...
```python
# 定义一个两组样本均值检验函数，使它可以同时输出 t 检验与 Mann-Whitney U 检验的 p 值
...
```
两组样本均值的散点图可视化
![alt text](img/docs-CH6-attachment-image-6.png)
...
为了让读者更好地理解“成对”的概念，我们再把王老师的故事讲长一点。得知了王老师班的考试均值与自己班的没有显著差异，陈老师非常生气：“我堂堂优秀教师，居然不能和隔壁老王拉开差距，岂有此理！”，于是陈老师开始了为期一周的魔鬼训练，并在一个星期后又进行了一次全班的测验。陈老师**依旧抽取了上次十位同学的成绩**，依次为：
...
def paired_data(group1:np.ndarray,group2:np.ndarray,significance,alternative='two-sided'):
    ...
    # 配对t检验-样本差值服从正态分布
    _, pVal['t-test'] = stats.ttest_1samp(group2 - group1, 0, alternative=alternative)
    print('t-test------------------------')
    if pVal['t-test'] < significance:
      print(('在显著性水平{0:}下，两组配对样本的均值不相等(p={1:5.3f}).'.format(significance,pVal['t-test'])))
    else:
      print(('在显著性水平{0:}下，无法拒绝两组配对样本均值相等的假设.(p={1:5.3f})'.format(significance,pVal['t-test'])))
    ...
    if pVal['wilcoxon'] < significance:
      ...
      print(('在显著性水平{0:}下，无法拒绝两组配对样本均值相等的假设.(p={1:5.3f})'.format(significance,pVal['wilcoxon'])))
...
尽管在大样本下，非正态性数据的方差分析也是稳健的，但是在小样本下，对非正态性数据做方差分析还是可能存在误差。此时，我们可以使用kruskalwallis检验。该检验也是一种非参数检验，关于该检验的原理，我们就不再学习了。读者只需要知道它的应用场景既可。
...
我们用一个案例，带读者使用随机过程来对现实世界进行模拟分析与预测。在本次案例中，我们主要掌握如何使用简单的simpy进行仿真的流程，simpy的具体语法将在后续课程给读者详细解答，这里只需要直观感受下仿真的流程即可。
...
第3天，第17时的车辆数： 554
第3天，第21时的车辆数： 734
每天通过交通路口的车辆数之和为： [11491, 11014, 11572]
```
...
![alt text](img/docs-CH6-attachment-image-7.png)
...
```
```
10期转移后，状态分布为： [0.000e+00 7.000e-04 1.279e-01 8.714e-01]
```
...
```
![alt text](img/docs-CH6-attachment-image-8.png)
```python
# 柱状图
from plotnine import *
(
...
```
![alt text](img/docs-CH6-attachment-image-9.png)

读者现在可以先忽略可视化的代码部分，着重观察两个图表在表达信息上的差异。以上两个图其实都是表示：在gender、age和income上，不同类别的占比，但是第一个图明显比第二个图表达的信息更多，因为第一个图能表达每个类别的大致占比情况，而第二个图只能比较不同类别下的数量情况。
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-10.png)
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-11.png)
![alt text](img/docs-CH6-attachment-image-12.png)

现在，读者应该能体会到“Matplotlib的每一句代码都是往纸上添加一个绘图特征”这句话的含义了吧。现在，我们来看看这样的Matplotlib有什么优点与缺点。优点是非常简单易懂，而且能绘制复杂图表；缺点也是十分明显的，如果绘制复杂图表的时候一步一步地绘制，代码量还是十分巨大的。Seaborn是在Matplotlib的基础上的再次封装，是对Matplotlib绘制统计图表的简化。下面，我们一起看看Seaborn的基本绘图逻辑。
#### （2）Seaborn：
Seaborn 主要用于统计分析绘图，它在 Matplotlib 的基础上进行了更高层次的 API 封装。相比直接使用 Matplotlib，Seaborn 在统计图表绘制上更简洁，能够减少大量样式和参数设置。可以把 Seaborn 理解为 Matplotlib 在统计可视化方向上的补充。
下面，我们使用一个简单的例子，来看看使用 Seaborn 绘图与使用 Matplotlib 绘图之间的代码有什么不一样：
```python
# 准备数据
...
# 使用 Seaborn 绘制带有拟合直线效果的散点图
sns.lmplot(x="x", y="y", data=df)
```
![alt text](img/docs-CH6-attachment-image-13.png)
![alt text](img/docs-CH6-attachment-image-14.png)
可见，Seaborn把数据拟合等统计属性高度集成在绘图函数中，绘图的功能还是构筑在Matplotlib之上。因此，Seaborn相当于是完善了统计图表的Matplotlib工具库，二者应该是相辅相成的。因此，在实际的可视化中，我们往往一起使用Matplotlib和Seaborn，两者的结合应该属于Python的数据可视化的一大流派吧。
#### （3）Plotnine：
ggplot2 奠定了 R 语言数据可视化在 R 语言数据科学中的地位。提到 R 语言数据可视化，很多人首先想到的就是 ggplot2。数据可视化一直是 Python 的短板，即使有 Matplotlib、Seaborn 等数据可视化包，也很难在复杂图表的表达效率上完全接近 ggplot2。原因在于，当绘制复杂图表时，Matplotlib 和 Seaborn 由于“每一句代码都是往纸上添加一个绘图特征”的特性，往往需要大量代码语句。Plotnine 可以看作 ggplot2 在 Python 上的移植版，使用的基本语法与 R 语言 ggplot2 语法高度相似，使得 Python 的数据可视化能力大幅提升。为什么 ggplot2 和 Plotnine 会更适合数据可视化呢？原因可以类似于 Photoshop 绘图和 PPT 绘图的区别：PPT 更像一笔一画地绘图，而 Photoshop 使用“图层”的概念；在 Plotnine 中，每一句代码相当于往图像中添加一个图层，一个图层就是一类绘图动作，这会大大减少复杂可视化的组织成本，也更符合绘图者的认知。

下面，我们通过一个案例来看看Plotnine的图层概念以及Plotnine的基本绘图逻辑：
```python
from plotnine import *     # 将 Plotnine 所有模块引入
from plotnine.data import *   # 引入 Plotnine 自带数据集
mpg.head()
```
...
print(p1)   # 展示p1图像
```
![alt text](img/docs-CH6-attachment-image-15.png)

从上面的案例，我们可见 Plotnine 的绘图逻辑是：一句话一个图层。因此，在 Plotnine 中用少量代码就能画出较复杂的图表，这类似于 Photoshop 能通过图层轻松组织复杂图像，而 PPT 需要更多手工操作才能接近同样的效果。
...
但是值得注意的是，这里所说的绘制 ggplot 风格，是看起来像 ggplot 图表，但是 Matplotlib 本身仍然不具备 ggplot2 / Plotnine 那种完整的图层语法。

可视化工具选择可以先按任务判断：

| 场景 | 优先工具 | 原因 |
| --- | --- | --- |
| 快速画基础图、需要极坐标或高度自定义 | Matplotlib | 底层能力强，图形控制细 |
| 统计探索、回归趋势、分布图、箱线图 | Seaborn | 对统计图表封装好，代码更短 |
| 类别比较、分组关系、复杂直角坐标图层叠加 | Plotnine | 图层语法清晰，适合表达多变量映射 |

### 6.4.2  基本图表 Quick Start
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-16.png)
```python
...
print(p_single_bar)
```
![alt text](img/docs-CH6-attachment-image-17.png)
```python
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-18.png)
```python
...
print(p_mult_bar)
```
![alt text](img/docs-CH6-attachment-image-19.png)
```python
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-20.png)
```python
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-21.png)
```python
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-22.png)
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-23.png)
```python
...
print(p1)
```
![alt text](img/docs-CH6-attachment-image-24.png)
```python
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-25.png)
```python
# 使用Matplotlib绘制聚类散点图
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-26.png)
```python
# 使用plotnine绘制相关系数矩阵图：
...
print([p1, p2, p3])
```
![alt text](img/docs-CH6-attachment-image-27.png)
![alt text](img/docs-CH6-attachment-image-28.png)
![alt text](img/docs-CH6-attachment-image-29.png)
```python
# 使用Matplotlib/Seaborn绘制相关系数矩阵图
uniform_data = np.random.rand(10, 12)
sns.heatmap(uniform_data)
```
![alt text](img/docs-CH6-attachment-image-30.png)
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-31.png)
```python
# 使用matplotlib绘制箱线图
import seaborn as sns
from plotnine.data import mtcars
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-32.png)
```python
# 使用Matplotlib绘制饼状图：
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-33.png)
```python
# 使用Matplotlib绘制环状图：
from matplotlib import cm, colors
df = pd.DataFrame({
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-34.png)
...
print(p1)
```
![alt text](img/docs-CH6-attachment-image-35.png)
```python
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-36.png)
```python
# Plotnine绘制多系列折线图
...
print(p1)
```
![alt text](img/docs-CH6-attachment-image-37.png)
```python
# Matplotlib 绘制多系列折线图
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-38.png)
## 6.5  插值模型
...
| 方法 | 基本思想 | 适合场景 | 主要风险 |
| --- | --- | --- | --- |
| 线性插值 | 相邻两点之间用直线连接 | 数据变化较平缓、只需局部填补 | 曲线不够光滑 |
| 三次样条插值 | 分段三次多项式，并保持函数值、一阶导、二阶导连续 | 希望曲线平滑，且数据点较可靠 | 对异常点敏感 |
| 拉格朗日插值 | 用所有点构造一个整体插值多项式 | 理论推导或点数较少的插值演示 | 点数多时容易振荡 |
...
plt.show()
```
![alt text](img/docs-CH6-attachment-image-39.png)
