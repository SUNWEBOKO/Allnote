...
本章的学习目标是完成一条可观察的验证链：先读出器件身份，再擦除测试区域，写入已知数据，最后读回并比较。课程提供两个工程：

```text
D:\Elec\STM32入门教程资料\STM32Project-有注释版\11-1 软件SPI读写W25Q64
D:\Elec\STM32入门教程资料\STM32Project-有注释版\11-2 硬件SPI读写W25Q64
```

两个工程都使用 `W25Q64.c/.h` 的上层接口，差别集中在 `MySPI.c/.h`：软件工程用 GPIO 翻转时钟，硬件工程使用 SPI1 外设。这样可以先验证器件指令，再替换字节交换方式。
...
`W25Q64_WaitBusy()` 判断的是状态寄存器 1 的最低位：`BUSY=1` 表示内部操作仍在进行，`BUSY=0` 才能继续。课程工程已经加入计数超时；超时后只是退出等待，实际产品还应返回错误或记录故障，不能把“退出循环”当成“写入成功”。
...
课程中的 `W25Q64_PageProgram(Address, DataArray, Count)` 是单页编程接口，不会自动跨页拆分。调用前必须满足：

```text
0 < Count <= 256
(Address % 256) + Count <= 256
```

例如页内偏移为 `250` 时，最多只能写入 `6` 字节。需要写入更长数据时，应先写满当前页，再从下一页边界继续写入：

```c
uint32_t Offset = Address % 256;
uint16_t FirstCount = 256 - Offset;

W25Q64_PageProgram(Address, DataArray, FirstCount);
W25Q64_PageProgram(Address + FirstCount,
                   DataArray + FirstCount,
                   Count - FirstCount);
```

上例只演示跨一个页的情况；通用驱动还应循环处理多个页，并在每次页编程后等待 `BUSY` 清零。若调用者无法保证条件，应在驱动接口中增加参数检查，而不是依赖芯片的回绕行为。
